...
Root locus diagrams are graphical tools used to assess the stability of a system as one of its parameters varies -- usually the controller gain $K_c$. Here, we'll look at one system from homework. Consider the following open-loop transfer function, a process controller with PI-control:

$$L(s) = \frac{\frac{K_{c}}{1.5}(1.5s+1)}{s(2s+1)(s+1)}$$

Written in this form, we can see that $\tau_I = 1.5$, and we have poles @ $s = 0, -1/2, -1$
...
ctrl.root_locus_plot(Gol, initial_gain = 0);
```
